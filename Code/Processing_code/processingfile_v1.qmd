...
library(tidyverse)
library(readr)
data_location <-"C:/Users/taylo/Documents/GitHub/myrepo/project1/Chiyoko-Rclass-Project/Data/Raw_data/penguins_raw_dirty.csv"
data_path <- "C:/Users/taylo/Documents/GitHub/myrepo/project1/Chiyoko-Rclass-Project/Data/Raw_data/"
```
...
unique(d1$Species)   # Adelie fixed

oo<-grep("Pygoscelis papua", d1$Species)
d1$Species[oo] <- "Gentoo"
unique(d1$Species)  #Fixed Gentoo

pp<-grep("Pygoscelis antarctica", d1$Species)
d1$Species[pp] <- "Chinstrap"
unique(d1$Species) #Chinstrap fixed
```
...
```{r}
plot(d3$`Culmen Depth (mm)`, d3$`Culmen Length (mm)`)
plot(d3$`Flipper Length (mm)`, d3$`Culmen Length (mm)`)
plot(d3$`Flipper Length (mm)`, d3$`Culmen Depth (mm)`)
plot(d3$`Culmen Depth (mm)`, d3$`Body Mass (g)`)
