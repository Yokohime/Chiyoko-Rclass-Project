...
data_location <- "Chiyoko-Rclass-Project/penguins_raw_dirty.csv"
data_path <- "Users/taylo/Documents/Github/myrepo/project1/penguin.dictionary.csv"
library(readr)
rawdata <- read_csv("Chiyoko-Rclass-Project/penguins_raw_dirty.csv")
View(rawdata)
#load data. 
...
library(readr)
dictionary <- read_csv("penguin.dictionary.csv")
print(dictionary)
...
gg <- grep( "(Pygoscelis papua)" , d1$Species)
d1$Species[gg] <- "Gentoo penguin (Pygoscelis papua)"
unique(d1$Species)

hh <- grep( "(Pygoscelis antarctica)" , d1$Species)
d1$Species[hh] <- "Chinstrap penguin (Pygoscelis antarctica)"
...
qq<- grep( "(Pygoscelis adeliae)" , d1$Species)
d1$Species[qq] <- "Adelie"
unique(d1$Species)

ww<- grep( "(Pygoscelis papua)" , d1$Species)
d1$Species[ww] <- "Gentoo"
unique(d1$Species)

ee <- grep( "(Pygoscelis antarctica)" , d1$Species)
d1$Species[ee] <- "Chinstrap"
